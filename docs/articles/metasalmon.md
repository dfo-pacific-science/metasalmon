...
`fraser-coho-2024-sdp`. In interactive use it can also mention when a
...
The fuller table is an official Open Government Canada NuSEDS slice from
the Fraser and BC Interior workbook, filtered to `SPECIES == "Coho"` and
`ANALYSIS_YR %in% c(2023, 2024)`. It keeps a compact column subset and
uses `NATURAL_ADULT_SPAWNERS` because `NATURAL_SPAWNERS_TOTAL` is blank
for this two-year official slice. See `example-data-README.md` for
provenance, row counts, and the Open Government Licence - Canada note.
