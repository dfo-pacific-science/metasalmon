...
existing non-empty IRI values. Table-level suggestions are still
available when table metadata needs them, while code-level suggestions
default to factor/categorical source columns only. Use
...
Replace those placeholders before publishing. The `metadata/*.csv` files
