...
pipeline. It now also covers the package-native LLM review path inside
[`suggest_semantics()`](https://dfo-pacific-science.github.io/metasalmon/reference/suggest_semantics.md),
where the model judges only the retrieved shortlist rather than
inventing IRIs from scratch.
...
**Prompt helper for DwC-DP exports**

    Use metasalmon as the canonical semantic source. Propose ontology IRIs first. If asked for DwC export hints, include the DwC-DP mappings from suggest_semantics(..., include_dwc = TRUE); do not replace the ontology suggestions.
...
### Step 3c: Ask `metasalmon` to run the LLM review directly

If you want the package to call an LLM directly after retrieval, keep
the deterministic shortlist and turn on the optional assessment layer.

``` r

# Optional local context files: methods note, README, PDF report, etc.
context_files <- c("README.md", "methods-report.pdf")

suggested_llm <- suggest_semantics(
  df = df,
  dict = dict,
  llm_assess = TRUE,
...
suggestions_llm <- attr(suggested_llm, "semantic_suggestions")
assessments_llm <- attr(suggested_llm, "semantic_llm_assessments")

# Review-only by default; explicit application step:
dict <- apply_semantic_suggestions(
  suggested_llm,
  strategy = "llm",
...
- `llm_provider = "openai"` uses the default OpenAI-compatible path and
  reads `OPENAI_API_KEY` unless you pass `llm_api_key`.
- `llm_provider = "openrouter"` uses `https://openrouter.ai/api/v1` and
  reads `OPENROUTER_API_KEY` unless you pass `llm_api_key`.
- OpenRouter free models work by passing the model id through unchanged
  (for example a model name ending in `:free`).
- PDF context files require the optional `pdftools` package.

This keeps the safe default intact:
[`find_terms()`](https://dfo-pacific-science.github.io/metasalmon/reference/find_terms.md)
remains the candidate generator, and the LLM only judges the shortlist.
...
This advanced/manual writer places the canonical metadata tables under
`metadata/`, the table CSVs under `data/`, and writes `datapackage.json`
at the package root.
...
### Advanced: Phase 4 matching quality features

[`find_terms()`](https://dfo-pacific-science.github.io/metasalmon/reference/find_terms.md)
includes several features that improve match quality:

**Role-aware query expansion**: Queries are automatically expanded based
on role context. For example, searching for “kg” with `role = "unit"`
will also search for “kilogram”. Disable with `expand_query = FALSE` if
you need exact matching only.

``` r

# With expansion (default)
find_terms("kg", role = "unit", sources = sources_for_role("unit"))

# Without expansion
find_terms("kg", role = "unit", sources = sources_for_role("unit"), expand_query = FALSE)
```

**Cross-source agreement**: Terms appearing in multiple sources receive
score boosts. Check the `agreement_sources` column to see how many
sources returned the same term.
...
**Diagnostics for debugging**: When searches return unexpected results,
inspect the diagnostics attribute:
...
**Optional embedding rerank**: For advanced users, enable experimental
embedding-based reranking via the `METASALMON_EMBEDDING_RERANK=1`
environment variable. This is a placeholder for future
sentence-embedding reranking over lexical candidates.
...
# 5. Create package
write_salmon_datapackage(resources, dataset_meta, table_meta, dict, ...)
```
