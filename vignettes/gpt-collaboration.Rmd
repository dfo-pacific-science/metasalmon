---
title: "Using AI to Document Your Data"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Using AI to Document Your Data}
...
`suggest_semantics()`, where the model judges only the retrieved
shortlist rather than inventing IRIs from scratch.

The preferred workflow is now package-native rather than "export to
ChatGPT": create the package, review the created CSV files in Excel,
resolve `REVIEW:` markers, rebuild EDH XML if needed, validate, then
publish.

**See also:**
- The README's "How It Fits Together" section for the full picture
- [Publishing Data Packages](data-dictionary-publication.html) for the metadata and publication workflow
- [Linking to Standard Vocabularies](reusing-standards-salmon-data-terms.html) for the plain-language ontology guidance

**Canonical output contract:** Use `smn-gpt/SYSTEM-PROMPT.md` as the authoritative format rules (CSV only).

## Safe data checklist

- Share 50–500 representative rows, not the full dataset.
- Include column summaries (types, missingness, and unique values for categoricals).
...
- The latest DFO Salmon Ontology file (e.g., `dfo-salmon.ttl`) as a schema-only vocabulary source (not data)
...
- The ontology repository’s term request template (optional, but helps the model draft issues in the expected format):  
...
- Suggest IRIs from the Salmon Domain Ontology first (or other appropriate vocabularies when needed)
- Identify categorical fields that need code lists
- Propose controlled vocabulary values
- Return results in a strict, copy/pasteable CSV format matching the `metasalmon` schemas (see `column_dictionary.csv` and `codes.csv`)
...
**Example prompt:**
    ...
    2. Suggesting IRIs from the Salmon Domain Ontology first (or other appropriate vocabularies when needed) for:
       ...
       - If there is no reasonable match, tell me how to request a new term be added to the shared salmon-domain ontology first
...
`smn-gpt` uses the same I-ADOPT catalogue (symlinked into the repo) and follows the same rules: measurement columns require `term_iri`, `property_iri`, `entity_iri`, `unit_iri`, with optional `constraint_iri`.

### Step 3c: Ask `metasalmon` to run the LLM review directly

If you want the package to call an LLM directly after retrieval, keep the deterministic shortlist and turn on the optional assessment layer.
...
- `llm_provider = "openai"` uses the default OpenAI-compatible path and reads `OPENAI_API_KEY` unless you pass `llm_api_key`.
- `llm_provider = "openrouter"` uses `https://openrouter.ai/api/v1` and reads `OPENROUTER_API_KEY` unless you pass `llm_api_key`.
- If you do not specify `llm_model` for OpenRouter, `metasalmon` defaults to `openrouter/free`.
- OpenRouter free models also work by passing a specific model id through unchanged (for example a model name ending in `:free`).
- PDF context files require the optional `pdftools` package.

This keeps the safe default intact: `find_terms()` remains the candidate generator, and the LLM only judges the shortlist.
...
This advanced/manual writer places the canonical metadata tables under `metadata/`, the table CSVs under `data/`, and writes `datapackage.json` at the package root.
...
The shared salmon-domain ontology is still evolving, so it's normal that many terms you want will not exist yet.
...
- Capture what you *wanted* as a proposed new term in `gpt_proposed_terms.csv` and include its parent relationship:
...
- Draft a GitHub issue for the ontology repository using the new-term template. Default to the shared salmon-domain ontology unless the term is clearly DFO policy/operations-specific:  
  <https://github.com/salmon-data-mobilization/salmon-domain-ontology/blob/main/.github/ISSUE_TEMPLATE/new-term-request.md>
...
7.  **Keep deterministic**: Keep prompts in version control, ask for strict output (CSV only), prefer "merge by keys" workflows, and (if using an API) use a low temperature.
...
function provides role-aware IRI suggestions using the bundled I-ADOPT terminology catalogue and external ontology services (GCDFO first for salmon-domain roles, then OLS/NVS/BioPortal as needed):
...
This complements the package-native review workflow by providing deterministic, reproducible suggestions that you can review and merge into your dictionary. In the current preferred path, `create_sdp(..., llm_assess = TRUE)` writes the package, prefixes LLM-selected draft IRIs with `REVIEW:`, and sends you back to the package for manual cleanup in Excel. See [Linking to Standard Vocabularies](reusing-standards-salmon-data-terms.html) for details on `find_terms()` and the I-ADOPT catalogue.

### Advanced: Phase 4 matching quality features

`find_terms()` includes several features that improve match quality:

**Role-aware query expansion**: Queries are automatically expanded based on role context. For example, searching for "kg" with `role = "unit"` will also search for "kilogram". Disable with `expand_query = FALSE` if you need exact matching only.
...
**Cross-source agreement**: Terms appearing in multiple sources receive score boosts. Check the `agreement_sources` column to see how many sources returned the same term.
...
**Diagnostics for debugging**: When searches return unexpected results, inspect the diagnostics attribute:
...
**Optional embedding rerank**: For advanced users, enable experimental embedding-based reranking via the `METASALMON_EMBEDDING_RERANK=1` environment variable. This is a placeholder for future sentence-embedding reranking over lexical candidates.
...
- **DFO Salmon Ontology**: <https://w3id.org/gcdfo/salmon/> (canonical term IRIs use `https://w3id.org/gcdfo/salmon#<Term>`)
  ...
  - Repository: <https://github.com/dfo-pacific-science/dfo-salmon-ontology>
