...
# End-to-end semantic enrichment (I-ADOPT-aware)

Combine GPT assistance with `metasalmon` helpers to fill the I-ADOPT component fields that measurement columns require:

```{r}
# Use the bundled minimal example dictionary
dict <- readr::read_csv(
  system.file("extdata", "column_dictionary.csv", package = "metasalmon"),
  show_col_types = FALSE
)

# Ask metasalmon for best-effort suggestions using role-aware ranking
dict <- suggest_semantics(
  df = NULL,                # optional, not used today
  dict = dict,
  sources = c("ols", "nvs") # public, no key required; add "bioportal" if you set BIOPORTAL_APIKEY
)

suggestions <- attr(dict, "semantic_suggestions")
head(suggestions)

# Review suggestions per role (variable/property/entity/unit/constraint/method) and paste accepted IRIs
dict$term_iri[dict$column_name == "NATURAL_SPAWNERS_TOTAL"] <- suggestions$iri[suggestions$dictionary_role == "variable"][1]
dict$property_iri[dict$column_name == "NATURAL_SPAWNERS_TOTAL"] <- suggestions$iri[suggestions$dictionary_role == "property"][1]
dict$entity_iri[dict$column_name == "NATURAL_SPAWNERS_TOTAL"] <- suggestions$iri[suggestions$dictionary_role == "entity"][1]
dict$unit_iri[dict$column_name == "NATURAL_SPAWNERS_TOTAL"] <- suggestions$iri[suggestions$dictionary_role == "unit"][1]

validate_dictionary(dict)
```

If you also use `smn-gpt`, give GPT the same dictionary and data sample; the system prompt follows the same rules (measurement columns need `term_iri`, `property_iri`, `entity_iri`, `unit_iri`, with optional `constraint_iri`/`method_iri`) and prefers vocabularies from the I-ADOPT terminology catalogue that ships with `metasalmon`.
...
5. **Link to ontology**: Add IRIs to connect your data to the DFO Salmon Ontology for interoperability. Key IRIs include:
   - `https://w3id.org/gcdfo/salmon#ConservationUnit` - Conservation Units
   - `https://w3id.org/gcdfo/salmon#Stock` - Stock entities
   - `https://w3id.org/gcdfo/salmon#EscapementMeasurement` - Escapement measurements
   - `https://w3id.org/gcdfo/salmon#BroodYear` - Brood year references
   - `https://w3id.org/gcdfo/salmon#CatchYear` - Catch year references
   - `https://w3id.org/gcdfo/salmon#WSPBiologicalStatusZoneScheme` - Status zone scheme (Green/Amber/Red)
   - `https://w3id.org/gcdfo/salmon#SalmonOriginScheme` - Origin scheme (natural/hatchery)
   - See the full ontology at https://w3id.org/gcdfo/salmon for all available terms
