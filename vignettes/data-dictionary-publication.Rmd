...
This guide follows the 5-Minute Quickstart and focuses on the **publishing hardening** steps.
...
article covers the more explicit, manual workflow where you assemble the
metadata tables yourself and then call `write_salmon_datapackage()`.
...
[Describe the dataset and tables](#3-describe-the-dataset-and-tables).
...
`edh_build_iso19139_xml()` now defaults to the richer HNAP-aware EDH export and
still offers the older compact ISO 19139 path as an explicit fallback.
...
The default HNAP-aware path recognizes extra optional columns when present,
...
- Try the [Using AI to Document Your Data](gpt-collaboration.html) workflow for drafting descriptions and ontology-aligned metadata quickly.
